...
#' 
#' @export 
fs_perim_sd <- function(X, index=NULL, unit=NULL, col=NULL) UseMethod("fs_perim_sd")


#' @name fs_perim_sd
#' @export
fs_perim_sd.sp <- function(X, index=NULL, unit=NULL, col=NULL){
  X <- sf::st_as_sf(X)
  
  result <- fs_perim_sd(X, index, unit, col)
  return(result)
}
