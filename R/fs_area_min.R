#' Building area minimum calculation
#' 
...
#' 
#' @export 
fs_area_min <- function(X, 
                        index=NULL, 
                        unit=NULL, 
                        col=NULL) UseMethod("fs_area_min")


#' @name fs_area_min
#' @export
fs_area_min.sp <- function(X, index=NULL, unit=NULL, col=NULL){
  X <- sf::st_as_sf(X)
  
  result <- fs_area_min(X, index, unit, col)
  return(result)
}
...
#' @name fs_area_min
#' @export
fs_area_min.sf <- function(X, index=NULL, unit=NULL, col=NULL){
  if(!is.null(col)){
    if(!col %in% names(X)){
      ...
      result <- fs_area_min_calc(X, index, unit)
    }
  } else{
...
fs_area_min_calc <- function(X, index=NULL, unit=NULL){
  ...
  colNam <- paste0("fs_area_", unit, "_min")
