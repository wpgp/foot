...
#' 
#' @export 
fs_area_sd <- function(X, index=NULL, unit=NULL, col=NULL) UseMethod("fs_area_sd")


#' @name fs_area_sd
#' @export
fs_area_sd.sp <- function(X, index=NULL, unit=NULL, col=NULL){
  X <- sf::st_as_sf(X)
  
  result <- fs_area_sd(X, index, unit, col)
  return(result)
}
...
#' @name fs_area_sd
#' @export
fs_area_sd.sf <- function(X, index=NULL, unit=NULL, col=NULL){
  if(!is.null(col)){
    if(!col %in% names(X)){
      ...
      stop()
        ...
        result <- fs_area_sd_calc(X, index, unit)
    }
  } else{
...
fs_area_sd_calc <- function(X, index, unit=NULL){
