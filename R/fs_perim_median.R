#' Building perimeter median calculation
#' 
...
#' @import data.table
#' 
#' @aliases fs_perim_median
#' @rdname fs_perim_median
#' 
#' @export 
fs_perim_median <- function(X, 
                            index=NULL, 
                            unit=NULL, 
                            col=NULL) UseMethod("fs_perim_median")
 

#' @name fs_perim_median
#' @export
fs_perim_median.sp <- function(X, index=NULL, unit=NULL, col=NULL){
  X <- sf::st_as_sf(X)
  
  result <- fs_perim_median(X, index, unit, col)
  return(result)
}
...
#' @name fs_perim_median
#' @export
fs_perim_median.sf <- function(X, index=NULL, unit=NULL, col=NULL){
  if(!is.null(col)){
    if(!col %in% names(X)){
      ...
      result <- fs_perim_median_calc(X, index, unit)
    }
  } else{
...
fs_perim_median_calc <- function(X, index=NULL, unit=NULL){
  ...
  colNam <- paste0("fs_perim_", unit, "_median")
