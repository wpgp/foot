...
#' 
#' @export 
fs_compact_mean <- function(X, index=NULL, unit=NULL, col=NULL) UseMethod("fs_compact_mean")
...
fs_compact_mean_calc <- function(X, index, unit=NULL){
  ...
  DT[, compact := (4 * pi * a) / (p^2)]
  units(DT$compact) <- NULL
  
  colNam <- "fs_compact_mean"
  result <- DT[, setNames(.(mean(compact)), colNam), by=index]
  return(result)
}
