...
For example:
```{r include=FALSE}
# local file paths used for demonstration
...
provide a character or list list of character names of characteristics and
summary functions, respectively. The available metrics can be retrieved with
`list_fs()`.
...
(`outputTag=`). The tag is appended to the beginning of each file name. This can
be useful for identifying different outputs. These options are demonstrated in
...
It is also possible to calculate and summarise building footprints from within a
local, circular window. The output is still associated with each template grid
...
```{r, tidy=FALSE, Fig4, fig.cap="Total building perimeter in KM", fig.height=6, fig.width=6}
