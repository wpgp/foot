...
* "buildings" - polygons of building footprpints in `sf` format. Contains 8480
records. 
* "mastergrid" - geoTiff `RasterLayer` aligned to WorldPop datalayers 
* "adminZones" - 34 polygons in `sf` format for zonal statistics 
* "clusters" - 10 small polygons in `sf` format for sample sites

Note that the adminZones and cluster boundaries are purely artificial and for
...
All function names for calculating metrics begin with `fs_` (short for footprint
statistic). The second part of the name includes the group of measures (e.g.
area or perimeter), and finally an abbreviation for the summary statistic (e.g.
'cv' for coefficient of variation or 'sd' for standard deviation).
...
column for the index and the named column for summarised footprint metric.

All functions follow the same template, and, because the first argument is
always data, all the functions allow for "piping" syntax.
...
exposed in `foot`. Two exceptions are for calculating the nearest neighbour
distance and the rotated minimum bounding rectangle.
...
coordinate systems. Internally the calculations are done with a `data.table` to 
benefit from multithreading capabilities.
...
which has been rotated to minimise the area. In contrast a bounding box is alway
...
how much the angles of structures depart from a regular grid pattern.
...
(potentially uesful) side effects.
...
This clips the footprints, potentially leaving small slivers of structures in
the zone which will affect the feature statistics.

An additional side effect of the intersection is that overlapping zones are
allowed and this duplicates the footprints in both zones.
...
cells by the centroid point location. A fast approach is use cell numbers.
...
gridRes <- calculate_footstats(buildings, 
                               index=cellIDs, 
                               metrics=c("fs_perim_total","fs_count","fs_settled"),
                               gridded=F)
...
(@) Pixel-level calculation. While centroid-based calculations are fast, sometimes it may be 
necessary to know the true amount of settlement *per pixel*. The zonal index functions can be 
use to intersect buildings to grid squares, as shown.
```{r warning=FALSE}
...
can also be remerged to the ```stars``` or ```sf``` representation of the grid
and quickly rasterized based on geometry.

(@) Write gridded data to GeoTiff files. Using ```calculate_footstats``` a template raster and 
an output location can be specified. Each metrics will be written to a separate grid.
```{r}

```

The above approaches work fine for small study areas, but may be too slow over
large areas. Producing  country-scale gridded datasets with tools from
```foot``` is addressed in the second vignette ("bigfoot").
