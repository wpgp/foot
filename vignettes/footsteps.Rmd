...
the `col=` argument. While unlikely to be used often, 
...
At present, the `fs_mbr()` function is not vectorized because of how it relies
on `chull` and a loop is required to calculate all features. The angles can be
summarised as an entropy measure and normalised to describe how much the angles
of structures depart from a regular grid pattern (available in
`fs_angle_entropy`).
...
and supplied to the feature calculations as shown.
```{r warning=FALSE, tidy=F}
...
also demonstrates how an index of `sf` type can be provided to the wrapper
function.
```{r, fig.width=6, fig.height=6, tidy=F}
outputPath <- tempdir()
