...
library(raster)  # to demonstrate interaction with raster tools
...
single Geopackage file in WGS84 projection (EPSG:4326).
```{r}
...
provides access to the files's dimensions and other information.
...
in parallel. For this demonstration, one tile will be used.
```{r}
i <- 200 # example
```
...
The buffered tiles have an extra 3 pixels on each side that overlap into other
tiles but allow for additional footprints to be extracted to avoid edge effects.
```{r}
dim(mgTile)
dim(mgBuffTile)
```
